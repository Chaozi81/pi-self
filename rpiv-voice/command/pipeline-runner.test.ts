...
describe("startDictationPipeline — branch coverage", () => {
	...
	it("hallucination filter drops a repetition loop as an empty commit", async () => {
		// "ok ok ok ok" is a repetition loop caught by the filter.
		const recognize = vi.fn<SttEngine["recognize"]>().mockResolvedValue("ok ok ok ok");
	...
	it("filter toggle off lets the same phrase through", async () => {
		...
		expect(finalTranscript).toBe("ok ok ok ok");
		const committed = dispatched.filter((a) => a.kind === "audio_transcript_appended" && a.text === "ok ok ok ok");
