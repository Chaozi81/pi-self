...
// ── SenseVoice language hint, driven via getActiveLocale → createSttEngine ──
describe("handleVoiceCommand — sensevoice language hint", () => {
	beforeEach(() => {
		isModelDownloaded.mockReturnValue(true);
		assertModelIntact.mockReset();
		createSttEngine.mockReset().mockResolvedValue({
	...
	it("maps a supported locale (zh) to its base language", async () => {
		expect(await runOnceWithLocale("zh")).toBe("zh");
	});

	it("strips the region subtag for IETF tags (zh-CN → zh)", async () => {
		expect(await runOnceWithLocale("zh-CN")).toBe("zh");
	});
