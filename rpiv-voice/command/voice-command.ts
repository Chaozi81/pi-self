...
// Languages the bundled SenseVoice model recognizes natively. Mapped from
// i18n locale codes. When the active locale doesn't match, no hint is sent
// and the model falls back to auto-detection.
const SENSEVOICE_SUPPORTED_LANGUAGES: ReadonlySet<string> = new Set([
	"en",
	"ja",
	"ko",
	"yue",
...
function isSenseVoiceSupported(language: string): boolean {
	return SENSEVOICE_SUPPORTED_LANGUAGES.has(language);
}

function senseVoiceLanguageForLocale(locale: string | undefined): string | undefined {
	if (!locale) return undefined;
	const base = baseLanguage(locale);
	return isSenseVoiceSupported(base) ? base : undefined;
}
		...
		return await runWithSplash<Preflight>(
			...
			async (controller) => {
				...
				try {
					...
					// Pre-set SenseVoice's language hint from the active i18n locale
					// when we have a confident mapping; otherwise fall back to the
					// model's auto-detect. Pre-setting trades a small amount of
