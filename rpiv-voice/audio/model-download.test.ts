...
describe("getModelPaths", () => {
	it("returns paths under sense-voice/", () => {
		...
		expect(paths.tokensPath).toContain("sense-voice");
	});
});
...
describe("ensureModelDownloaded — failure rollback", () => {
	...
	it("wipes the model dir when fetch fails (network down)", async () => {
		...
		const wiped = vi
			.mocked(rmSync)
			.mock.calls.some((args) => args[0] === SENSE_VOICE_DIR && args[1]?.recursive === true);
...
describe("removeModelInstall", () => {
	it("rmSyncs the sense-voice dir recursively + force", () => {
		vi.mocked(rmSync).mockClear();
		removeModelInstall();
		expect(vi.mocked(rmSync)).toHaveBeenCalledWith(SENSE_VOICE_DIR, { recursive: true, force: true });
	});
});
