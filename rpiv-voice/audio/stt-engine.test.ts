...
describe("createSttEngine", () => {
	...
	it("uses provided paths in modelConfig.senseVoice", async () => {
		await createSttEngine(BASE_CONFIG);
		const config = MockOfflineRecognizer.mock.calls[0][0];
		expect(config.modelConfig.senseVoice.model).toBe("/models/model.int8.onnx");
		expect(config.modelConfig.tokens).toBe("/models/tokens.txt");
	});

	it("sets useITN to 1 for punctuation recovery", async () => {
		await createSttEngine(BASE_CONFIG);
		const config = MockOfflineRecognizer.mock.calls[0][0];
		expect(config.modelConfig.senseVoice.useITN).toBe(1);
	});
	...
	it("includes language in config when provided", async () => {
		await createSttEngine({ ...BASE_CONFIG, language: "zh" });
		const config = MockOfflineRecognizer.mock.calls[0][0];
		expect(config.modelConfig.senseVoice.language).toBe("zh");
	});

	it("omits language from config when undefined", async () => {
		await createSttEngine(BASE_CONFIG);
		const config = MockOfflineRecognizer.mock.calls[0][0];
		expect(config.modelConfig.senseVoice).not.toHaveProperty("language");
	});
});
